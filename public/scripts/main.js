...
$(".box").sortable({
  ...
  start: function (event, ui) {
    ...
    ui.item.attr("previousPadIndex", ui.item.closest(".box").attr("pad-index"));
  },
...
function refreshToyBox() {
  ...
  fetch("./json/toytags.json")
    ...
    .then((tags) => {
      tags.forEach((item) => {
        ...
        if (item.name !== "N/A" && item.index === -1) {
          document
            .querySelector("#toybox-tokens")
            .insertAdjacentHTML("beforeend", createItemHtml(item));
        } else if (item.index !== -1) {
          const pad = document.querySelector(`#toypad${item.index}`);
          if (pad) {
            pad.insertAdjacentHTML("beforeend", createItemHtml(item));
          }
        }
    ...
    .catch((err) => {
      console.error("Failed to load toytags:", err);
    });
}
